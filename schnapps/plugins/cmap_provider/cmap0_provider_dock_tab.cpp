...
CMap0Provider_DockTab::CMap0Provider_DockTab(SCHNApps* s, Plugin_CMapProvider* p) :
	schnapps_(s),
	plugin_(p),
...
void CMap0Provider_DockTab::selected_map_changed()
{
	...
	if (!currentItems.empty())
	{
		...
		selected_map_ = plugin_->cmap0(map_name);
	}
...
{
	if (!updating_ui_ && selected_map_)
		plugin_->remove_cmap0(selected_map_->name());
}
...
void CMap0Provider_DockTab::selected_map_cells_set_added(cgogn::Orbit orbit, const QString& name)
{
	...
	{
		case CMap0::Vertex::ORBIT:
			cs = selected_map_->cells_set(CMap0::Vertex::ORBIT, name);
			if (cs)
...
void CMap0Provider_DockTab::refresh_ui()
{
	...
	if (selected_map_)
	{
		...
		selected_map_->foreach_cells_set(CMap0::Vertex::ORBIT, [&] (CMapCellsSetGen* cells_set)
		{
