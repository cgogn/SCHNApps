...
#include <schnapps/plugins/cmap_provider/cmap2_cells_set.h>
...
CMap2Provider_DockTab::CMap2Provider_DockTab(SCHNApps* s, Plugin_CMapProvider* p) :
	schnapps_(s),
	plugin_(p),
...
void CMap2Provider_DockTab::selected_map_changed()
{
	...
	if (!currentItems.empty())
	{
		...
		selected_map_ = plugin_->cmap2(map_name);
	}
...
{
	if (!updating_ui_ && selected_map_)
		plugin_->remove_cmap2(selected_map_->name());
}
