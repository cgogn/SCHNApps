...
void Remeshing_Dialog::map_added(CMap2Handler *mh)
{
	updating_ui_ = true;
...
void Remeshing_Dialog::map_removed(CMap2Handler *mh)
{
	if (selected_map_ == mh)
	{
		disconnect(selected_map_, SIGNAL(attribute_added(cgogn::Orbit, const QString&)), this, SLOT(selected_map_attribute_added(cgogn::Orbit, const QString&)));
...
void Remeshing_Dialog::selected_map_attribute_removed(cgogn::Orbit orbit, const QString& attribute_name)
{
	if (orbit == CMap2::Vertex::ORBIT)
	{
		QString vec3_type_name = QString::fromStdString(cgogn::name_of_type(VEC3()));
...
void Remeshing_Dialog::refresh_ui()
{
	updating_ui_ = true;
